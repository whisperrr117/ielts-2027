...
(function () {
  ...
  function getPriority() {
    ...
    const top = scored[0];
    let reason = "";
    if (manual && manual === top.k) reason = "你手动标记为本阶段重点";
    else if (top.gap >= 7) reason = `已经 ${top.gap} 天没有完整训练`;
    else if (top.count === 0) reason = "过去 7 天训练次数偏少";
  ...
  function buildWorkPlan(dateStr, energy) {
    ...
    if (energy === "exhausted") {
      tasks.push(
        makeTask({
          key: focus === "Writing" ? "Speaking" : focus,
          title: `${ICONS[focus] || "🌿"} 保底 · ${focus === "Writing" ? "Listening" : focus}`,
          mins: 10,
          steps: [
            "打开任意英语材料（音频 / 文章 / ChatGPT）",
            "接触 10 分钟即可，不要求完整做题",
          ],
          doneStd: "英语今天出现过",
          tier: "min",
    ...
    if (energy === "tired") {
      // 2 modules ~25 min
      if (focus === "Speaking" || focus === "Writing") {
        tasks.push(
          makeTask({
            key: "Speaking",
            title: "🗣 Speaking · 轻量",
        ...
        tasks.push(
          makeTask({
            ...
            title: "🎧 Listening · 轻量",
            mins: 10,
            steps: ["听一小段（播客 / 真题片段）", "标出 1–2 个没听清的地方"],
            doneStd: "听完并找出至少 1 个难点",
            tier: "light",
          })
        );
      } else {
        tasks.push(
          makeTask({
            key: focus,
            title: `${ICONS[focus]} ${focus} · 轻量`,
            mins: 15,
            steps:
              focus === "Listening"
                ? ["听一小段材料", "跟读或复述一句"]
                : ["读一篇短文 / 半篇 passage", "用一句话概括主旨"],
            doneStd: "完成指定片段即可",
            tier: "light",
          })
        );
        tasks.push(
          makeTask({
            ...
            title: "🔁 Review · 表达",
            mins: 10,
            steps: ["打开表达库，选 3 个表达", "每个口头造句一次"],
            doneStd: "至少主动说出 3 个表达",
            tier: "light",
          })
        );
      }
    ...
    tasks.push(
      makeTask({
        key: "Review",
        title: "🔁 Review · 表达 10 min",
        mins: 10,
        steps: [
          "复习最近 Speaking 中 3–5 个表达",
          "每个至少主动造句 / 说出来一次",
        ],
  ...
  function renderHome() {
    ...
    let goalText = "";
    if (energy === "off") {
      goalText = "今天可以完全休息。若方便，随意接触一点英语即可——不是任务。";
    } else if (mode === "rest") {
      goalText =
        energy === "tired"
          ? "建议学习：约 1–1.5 小时。目标：完成核心产出 + 一段输入，不求四科全做。"
          : "建议学习：2–3 小时。目标：完成一次完整能力训练（有产出），不必四科全满。";
    } else {
      if (energy === "exhausted") goalText = "建议学习：10–15 分钟。目标：让英语今天出现过即可。";
      else if (energy === "tired") goalText = "建议学习：约 25 分钟。目标：低负荷保持连接。";
      else goalText = "建议学习：30–40 分钟。目标：保持英语连接，不追求大量输入。";
    }
    ...
    // focus banner
    document.getElementById("focusBanner").innerHTML =
      energy === "off"
        ? "今天不安排正式任务。明天按计划继续，不补课。"
        : `本周重点：<strong>${pri.focus}</strong><br>原因：${pri.reason}`;
    ...
    if (energy === "off" || state.skippedDays[t]) {
      document.getElementById("planTotal").textContent = "";
      box.innerHTML = `<p class="hint-text">今天采用休息模式。没有「失败」，也没有连胜要守。需要时随时把状态改回「正常」。</p>`;
      startBtn.style.display = "none";
    } else {
      // rebuild if old-format tasks (no steps)
      ...
      box.innerHTML = tasks
        .map((task) => {
          ...
          return `
          <div class="task ${isDone ? "done" : ""} ${isActive ? "active-now" : ""}" data-id="${task.id}">
            <div class="task-top">
              <div class="task-title">${task.title}</div>
              <span class="task-mins">${task.mins} min</span>
            </div>
            ${steps ? `<ul class="task-steps">${steps}</ul>` : task.desc ? `<div class="task-steps">${task.desc}</div>` : ""}
            ...
            ${
              isDone
                ...
                : `<div class="task-actions">
                    ...
                    <button type="button" class="btn btn-sm btn-soft btn-focus" data-id="${task.id}">开始这项</button>
                    ${task.key === "Speaking" ? `<button type="button" class="btn btn-sm btn-soft btn-save-expr">存表达</button>` : ""}
                  </div>`
            }
          </div>`;
        })
      ...
      startBtn.textContent = next ? `开始：${next.title.replace(/^[^ ]+ /, "").slice(0, 24)}` : "今日模块已完成";
    ...
    document.getElementById("weekSummary").textContent =
      `本周保持英语连接：${contact}/7 天（不是连胜，断一天也没关系）`;

    const maxC = Math.max(1, ...Object.values(pri.counts), 1);
    document.getElementById("balanceGrid").innerHTML = SKILLS.map((k) => {
