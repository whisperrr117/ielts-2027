...
(function () {
  ...
  function renderHome() {
    ...
    if (energy === "off" || state.skippedDays[t]) {
      ...
      // activeTaskId missing = default first incomplete; null = user cancelled (none expanded)
      const hasExplicit = Object.prototype.hasOwnProperty.call(state.activeTaskId, t);
      const activeId = hasExplicit
        ? state.activeTaskId[t]
        : (tasks.find((x) => !done.has(x.id)) || {}).id || null;

      box.innerHTML = tasks
        .map((task) => {
          const isDone = done.has(task.id);
          const isActive = !!activeId && task.id === activeId && !isDone;
            ...
            ${
              isDone
                ...
                ? `<div class="task-actions">
                    ...
                    <button type="button" class="btn btn-sm btn-soft btn-cancel-focus">取消</button>
      ...
      box.querySelectorAll(".btn-cancel-focus").forEach((b) => {
        b.onclick = () => {
          state.activeTaskId[t] = null;
          save();
          renderHome();
          toast("已取消");
        };
      ...
      } else if (activeId && next.id === activeId) {
  ...
  function renderExpr() {
    ...
    const STATUS_BTN = ["推进复习", "再推进 → 已使用", "已主动使用 ✓"];
    ...
    list.innerHTML =
      ...
      items
        .map((e) => {
          const st = e.reviewStatus || 0;
          return `
      <div class="list-item">
        <div class="main">
          ${e.example ? `<div class="example">例：${e.example}</div>` : ""}
          <div class="title">${e.zh || e.mine || "（无中文）"}</div>
          <div class="sub">我的：${e.mine || "—"}</div>
          <div class="sub">更自然：${e.natural || "—"}</div>
          <div class="sub">
            ${e.problemType || ""} ·
            <span class="status-${st}">${REVIEW_STATUS[st] || ""}</span> ·
            ${e.category || ""} · ${e.date || ""}
          </div>
        </div>
        <div style="display:flex;flex-direction:column;gap:4px">
          <button type="button" class="btn btn-sm btn-soft btn-adv-expr" data-id="${e.id}">${STATUS_BTN[st]}</button>
          <button type="button" class="btn btn-sm btn-soft btn-edit-expr" data-id="${e.id}">编辑</button>
          <button type="button" class="btn btn-sm btn-soft btn-del-expr" data-id="${e.id}">删</button>
        </div>
      </div>`;
        })
        .join("");

    list.querySelectorAll(".btn-del-expr").forEach((b) => {
      b.onclick = () => {
    ...
    list.querySelectorAll(".btn-edit-expr").forEach((b) => {
      b.onclick = () => openExprModal(b.dataset.id);
    });
  }

  function openExprModal(editId) {
    const existing = editId ? state.expressions.find((x) => x.id === editId) : null;
    ...
    openModal(
      ...
      <div class="field"><label>我的例句</label><input type="text" id="eEx" placeholder="用这个表达造一个完整句子" /></div>
      <div class="field"><label>分类</label>
        <select id="eCat">${EXPR_CATS.filter((c) => c !== "All")
          .map((c) => `<option>${c}</option>`)
          .join("")}</select>
      </div>
      <button type="button" class="btn btn-primary btn-block" id="eSave">${isEdit ? "保存修改" : "保存"}</button>
      `,
      (close) => {
        if (existing) {
          document.getElementById("eZh").value = existing.zh || "";
          document.getElementById("eMine").value = existing.mine || "";
          document.getElementById("eNat").value = existing.natural || "";
          document.getElementById("eEx").value = existing.example || "";
          if (existing.problemType) document.getElementById("eProb").value = existing.problemType;
          if (existing.category) document.getElementById("eCat").value = existing.category;
        }
        document.getElementById("eSave").onclick = () => {
          ...
          if (isEdit) {
            existing.zh = zh;
            existing.mine = mine;
            existing.natural = document.getElementById("eNat").value;
            existing.problemType = document.getElementById("eProb").value;
            existing.example = document.getElementById("eEx").value;
            existing.category = document.getElementById("eCat").value;
            toast("已更新");
          } else {
  ...
  function renderVocab() {
    ...
    list.innerHTML = state.vocab
      ...
      <div class="list-item">
        <div class="main">
          ${v.example ? `<div class="example">例：${v.example}</div>` : ""}
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          <button type="button" class="btn btn-sm btn-soft btn-edit-vocab" data-id="${v.id}">编辑</button>
          <button type="button" class="btn btn-sm btn-soft btn-del-vocab" data-id="${v.id}">删</button>
        </div>
      </div>`
      )
      .join("");
    list.querySelectorAll(".btn-del-vocab").forEach((b) => {
      b.onclick = () => {
    ...
    list.querySelectorAll(".btn-edit-vocab").forEach((b) => {
      b.onclick = () => openVocabModal(b.dataset.id);
    });
  }

  function openVocabModal(editId) {
    const existing = editId ? state.vocab.find((x) => x.id === editId) : null;
    const isEdit = !!existing;
    ...
    openModal(
      ...
      <button type="button" class="btn btn-primary btn-block" id="vSave">${isEdit ? "保存修改" : "保存"}</button>
      `,
      (close) => {
        if (existing) {
          document.getElementById("vWord").value = existing.word || "";
          document.getElementById("vMean").value = existing.meaning || "";
          document.getElementById("vEx").value = existing.example || "";
          document.getElementById("vMine").value = existing.mine || "";
          document.getElementById("vPos").value = existing.pos || "";
          document.getElementById("vTopic").value = existing.topic || "";
          document.getElementById("vFam").value = String(existing.familiarity ?? 0);
        }
        document.getElementById("vSave").onclick = () => {
          ...
          if (isEdit) {
            existing.word = word;
            existing.meaning = document.getElementById("vMean").value;
            existing.example = document.getElementById("vEx").value;
            existing.mine = document.getElementById("vMine").value;
            existing.pos = document.getElementById("vPos").value;
            existing.topic = document.getElementById("vTopic").value;
            existing.familiarity = Number(document.getElementById("vFam").value);
            toast("已更新");
          } else {
            state.vocab.push({
              id: uid(),
    ...
    if (!state.writingErrors.length) {
      ...
      list.innerHTML = state.writingErrors
        ...
        <div class="list-item">
          ...
          <div style="display:flex;flex-direction:column;gap:4px">
            <button type="button" class="btn btn-sm btn-soft btn-edit-we" data-id="${e.id}">编辑</button>
            <button type="button" class="btn btn-sm btn-soft btn-del-we" data-id="${e.id}">删</button>
          </div>
        </div>`
        )
        .join("");
      list.querySelectorAll(".btn-del-we").forEach((b) => {
        b.onclick = () => {
      ...
      list.querySelectorAll(".btn-edit-we").forEach((b) => {
        b.onclick = () => openWritingErrModal(b.dataset.id);
      });
    }
    ...
    if (!state.writings.length) {
      ...
      wlist.innerHTML = state.writings
        ...
        <div class="list-item">
          ...
          <button type="button" class="btn btn-sm btn-soft btn-del-w" data-id="${w.id}">删</button>
        </div>`
        )
        .join("");
      wlist.querySelectorAll(".btn-del-w").forEach((b) => {
        b.onclick = () => {
  ...
  function openWritingErrModal(editId) {
    const existing = editId ? state.writingErrors.find((x) => x.id === editId) : null;
    const isEdit = !!existing;
    ...
    openModal(
      ...
      <button type="button" class="btn btn-primary btn-block" id="weSave">${isEdit ? "保存修改" : "保存"}</button>
      `,
      (close) => {
        if (existing) {
          if (existing.type) document.getElementById("weType").value = existing.type;
          document.getElementById("weDetail").value = existing.detail || "";
        }
        document.getElementById("weSave").onclick = () => {
          if (isEdit) {
            existing.type = document.getElementById("weType").value;
            existing.detail = document.getElementById("weDetail").value;
            toast("已更新");
          } else {
  ...
  function renderResources() {
    ...
    list.innerHTML = state.resources
      ...
      <div class="list-item">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ...
          <button type="button" class="btn btn-sm btn-soft btn-edit-res" data-id="${r.id}">编辑</button>
    ...
    list.querySelectorAll(".btn-edit-res").forEach((b) => {
      b.onclick = () => openResModal(b.dataset.id);
    });
  }

  function openResModal(editId) {
    const existing = editId ? state.resources.find((x) => x.id === editId) : null;
    const isEdit = !!existing;
    ...
    openModal(
      ...
      <button type="button" class="btn btn-primary btn-block" id="rSave">${isEdit ? "保存修改" : "保存"}</button>
      `,
      (close) => {
        if (existing) {
          document.getElementById("rName").value = existing.name || "";
          if (existing.type) document.getElementById("rType").value = existing.type;
          document.getElementById("rUrl").value = existing.url || "";
          document.getElementById("rSub").value = existing.subject || "";
          document.getElementById("rLevel").value = existing.level || "";
          document.getElementById("rPhase").value = existing.phase || "";
        ...
        document.getElementById("rSave").onclick = () => {
          ...
          if (isEdit) {
            existing.name = name;
            existing.type = document.getElementById("rType").value;
            existing.url = document.getElementById("rUrl").value;
            existing.subject = document.getElementById("rSub").value;
            existing.level = document.getElementById("rLevel").value;
            existing.phase = document.getElementById("rPhase").value;
            existing.note = document.getElementById("rNote").value;
            toast("已更新");
          } else {
